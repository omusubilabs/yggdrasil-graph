...
- [x] Finnish (`fi`) fully translated — `entities.json` and `relations.json`
      ...
      `src/i18n/config.ts`.
- [x] Icelandic (`is`) added as a partial locale — `ui.json` is translated;
      `entities.json` and `relations.json` still fall back to English.

Not built yet, roughly in the order it's worth picking them up:

- [ ] **next up** — Finish translating `is/entities.json` and
      `is/relations.json`, then promote `is` to `complete`.
- [ ] **after that** — Norwegian Bokmål, Swedish, Danish, in that order.
